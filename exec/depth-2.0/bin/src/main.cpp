...
int main(int argc,char* argv[]){
	...
	parser.set_arg("-cav","cav_sol_name","");
	...
	string cav_sol_name = parser.value("cav_sol_name");
	...
	for (unsigned int i = 0; i < iteration_n; ++i){
		...
		instance[i].set_parameter(survival_n, cavity, dimension[random_axis[i]], random_rotation[i], random_translation[i], radius_w, clash_dist, min_sol_name, cav_sol_name, unit_box_x, unit_box_y, unit_box_z);
	} // end for
	...
	for (unsigned int i = 0; i < mdl.size(); ++i){
		...
		fatom << mdl.serial(i) << "\t" << mdl.chainID(i) << ":" << mdl.resSeq(i) << mdl.iCode(i) << "\t" << mdl.name(i) << "\t" << num2string(round_double(ave_depth[i],2)) << "\t" << num2string(round_double(std_depth[i],2)) << endl;
	} // end for
	...
	for (unsigned int i = 0; i < mdl_depth.size(); ++i){
		mdl.set_T(i, residue_depth[mdl.chainID(i)+":"+mdl.resSeq(i)+mdl.iCode(i)]);
	} // end for
	mdl.write(bufferfname1);
	...
	if (cav_sol_name != ""){
		substitutor S;
		for (unsigned int i = 0; i < iteration_n; ++i){
			S.substitute(bufferfname1, cav_sol_name+"-"+num2string(i)+".coordinates", cav_sol_name+"-"+num2string(i)+".pdb");
		} // end for
	} // end if
	return 0;
