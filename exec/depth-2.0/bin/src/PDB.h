...
class PDB{
							...
							if (__iCode == "") {
								...
								if ( ( __altLoc == " ") || (__altLoc == "A") || (__altLoc == "1") ) {
									...
									if (read == 1){
										...
										_name.push_back(strip(line.substr(12, 16-12)));
	...
	PDB(string filename, vector <string> keywords){ // overload
			...
			if (line.size() >= 54){ // at least contains coordinates
				...
				try{
					...
					if (is_element(__atom, keywords)){
						...
						_charge   .push_back(strip(   line.substr(78, 80-78)));
						_residue_list.insert( make_pair(strip(line.substr(21, 22-21)), strip(line.substr(22, 26-22))+ strip(line.substr(26, 27-26))));
					} // end if
				} catch (exception& out_of_range){
					// do nothing
				} // end try
		...
		for (unsigned int i = 0; i < _x.size(); ++i){
			_residue[make_pair(_chainID[i], _resSeq[i] + _iCode[i])].push_back(i);
		} // end for
